                                ...
                                size_t channels)
{
    ...
    for (size_t z = 0; z < channels; z++)
    {
        ...
        grad_x -= CUDA_IMAGE_BUF_ACCESS(in, pitch, height, x - 1, y - 1, z);
        grad_x -= 2 * CUDA_IMAGE_BUF_ACCESS(in, pitch, height, x - 1, y, z);
        grad_x -= CUDA_IMAGE_BUF_ACCESS(in, pitch, height, x - 1, y + 1, z);
        ...
        grad_y -= CUDA_IMAGE_BUF_ACCESS(in, pitch, height, x - 1, y - 1, z);
        grad_y -= 2 * CUDA_IMAGE_BUF_ACCESS(in, pitch, height, x, y - 1, z);
        grad_y -= CUDA_IMAGE_BUF_ACCESS(in, pitch, height, x + 1, y - 1, z);
    ...
    grad /= channels; // get to range [-1, 1]
    grad = (grad + 1) / 2;

    CUDA_IMAGE_BUF_ACCESS(out, out_pitch, height, x, y, 0) = numeric_limits<PixelType>::max * grad;
}
                                   ...
                                   size_t height = 0)
{
    ...
    gridsize = dim3(-(-(ssize_t)(image.width - 2) / blocksize.x),
                    -(-(ssize_t)(image.height - 2) / blocksize.y));
